...
function albumsShow(req, res) {
  ...
  .exec((err, album) => {
    ...
    Track
    .find({album_name: id})
    .populate(['artist', 'album_name'])
    .exec((err, tracks) => {
      if (!tracks) return res.render('albums/show', { album: {}, error: 'No tracks were found' });
      return res.render('albums/show', { album, tracks, error: null });
    });
  });
}
